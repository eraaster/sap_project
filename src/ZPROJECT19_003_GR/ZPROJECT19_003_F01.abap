...
FORM CONTAINER1_FIELD.
  ...
  GS_FIELDCAT-SCRTEXT_M = '입고'.
  GS_FIELDCAT-CHECKBOX = 'X'. "체크박스 설정
  GS_FIELDCAT-EDIT = 'X'.
  ...
  GS_FIELDCAT-SCRTEXT_M = '구매처'.
  APPEND GS_FIELDCAT TO GT_FIELDCAT1.

  CLEAR : GS_FIELDCAT.
  GS_FIELDCAT-COL_POS = 5.
  GS_FIELDCAT-FIELDNAME = 'BEDAT'.
  GS_FIELDCAT-SCRTEXT_M = '증빙일'.
  GS_FIELDCAT-OUTPUTLEN = 10. "화면 너비
  APPEND GS_FIELDCAT TO GT_FIELDCAT1.

  CLEAR : GS_FIELDCAT.
  GS_FIELDCAT-COL_POS = 6.
  GS_FIELDCAT-FIELDNAME = 'ZMATNR'.
  GS_FIELDCAT-SCRTEXT_M = '자재번호'.
  APPEND GS_FIELDCAT TO GT_FIELDCAT1.
...
FORM get_data2 .
  ...
  "------------------------------------------------------------
  " 1) 구매오더 아이템(ZPDT19_005) 데이터 전체 가져오기
  "------------------------------------------------------------
  SELECT *
    FROM zpdt19_005
    WHERE ebeln = @p_ebeln
  ...
  LOOP AT lt_item INTO DATA(ls_item).
    ...
    IF lv_remain <= 0.
      MESSAGE '이미 모든 수량이 입고되었습니다.' TYPE 'S'.
      CONTINUE. " 이미 전량 입고된 품목은 스킵
    ENDIF.

    "------------------------------------------------------------
    " 4) 구매오더 헤더(ZPDT19_004)에서 기타 정보 가져오기
    "------------------------------------------------------------
    ...
    gs_001-menge   = lv_remain.          " ✅ 남은 수량으로 대체
    ...
    gs_001-remain_menge = lv_remain.  " 기준값
    gs_001-menge        = lv_remain.  " 화면용
...
FORM add_data .
  ...
  DATA: lv_mblnr TYPE mblnr,     "입고문서번호
        lv_mjahr TYPE mjahr,     "회계연도
        lv_zeile TYPE numc4,     "아이템번호
  ...
  IF lt_checked IS INITIAL.
    MESSAGE '입고 처리할 행이 선택되지 않았습니다.' TYPE 'S'.
    RETURN.
  ENDIF.

  " 입고문서번호 자동 채번(ZMKPF19)
...
*    lv_belnr = lv_belnr + 1.
  ...
  IF sy-subrc <> 0.
    MESSAGE '오류가 발생했습니다. 다시 시도해주세요.' TYPE 'E'.
  ENDIF.
...
FORM delete_data .
  ...
  DATA: lt_rows TYPE lvc_t_row,
        ...
        ls_data TYPE ty_004, "gt_004 구조
        ls_new TYPE ty_001, "gt_001 구조
        lv_cnt TYPE i.
  ...
  IF lt_rows IS INITIAL.
    MESSAGE '취소할 입고 라인을 선택하세요.' TYPE 'S'.
    RETURN.
  ENDIF.
...
FORM validate_line USING p_row TYPE ty_001
  ...
  IF p_row-menge IS INITIAL OR p_row-menge <= 0.
    MESSAGE e398(00)
      WITH |수량은 0보다 큰 숫자만 가능합니다. (품목 { p_row-ebelp })|.
    p_ok = abap_false.
    RETURN.
  ENDIF.
  ...
  IF p_row-menge > p_row-remain_menge.
     MESSAGE e398(00)
  WITH |입고 수량이 구매오더 잔여 수량을 초과했습니다 (잔여: { p_row-remain_menge }, 입력: { p_row-menge })|.
    p_ok = abap_false.
    RETURN.
  ENDIF.
