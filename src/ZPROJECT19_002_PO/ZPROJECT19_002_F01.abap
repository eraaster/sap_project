...
FORM get_data .
  ...
  IF lv_cnt = 0.
    MESSAGE '회사코드와 구매처 조합이 존재하지 않습니다.' TYPE 'E'.
    EXIT.
  ENDIF.
...
FORM GET_DATA2 .
  ...
  INNER JOIN zpdt19_005 AS B
    ON A~EBELN = B~EBELN "PO번호
 WHERE A~LIFNR = @p_lifnr "구매처코드
   AND A~EBELN = @p_ebeln
   AND A~BUKRS = @p_bukrs
...
FORM delete_data.
  ...
  IF lt_rows IS INITIAL.
    MESSAGE '삭제할 라인을 선택하세요.' TYPE 'I'.
    RETURN.
  ENDIF.

  " 2) 뒤에서부터 삭제해야 index 밀림 방지됨
...
FORM validate_line USING p_row TYPE ty_001
  ...
  IF p_row-prdat < p_bedat.
    MESSAGE '납품일은 증빙일보다 같거나 이후여야 합니다.' TYPE 'E'.
    p_ok = abap_false.
    RETURN.
  ENDIF.

  IF p_row-menge < 1.
    MESSAGE '수량은 1 이상의 정수여야 합니다.' TYPE 'E'.
    p_ok = abap_false.
    RETURN.
  ENDIF.

  IF p_row-menge <> trunc( p_row-menge ).
    MESSAGE '수량은 정수만 가능합니다.' TYPE 'E'.
    p_ok = abap_false.
    RETURN.
  ENDIF.
...
FORM add_item_line.
  ...
  "--------------------------------------------------------
  " 1) 정합성 전체 체크 (아이템 전체 검증 먼저)
  "--------------------------------------------------------
  LOOP AT gt_001 INTO gs_001.
    PERFORM validate_line USING gs_001 CHANGING lv_ok.
    IF lv_ok = abap_false.
      MESSAGE '정합성 오류 발생. 저장 중단.' TYPE 'E'.
      RETURN.
    ENDIF.
  ENDLOOP.

  "--------------------------------------------------------
  " 2) EBELN 자동 채번 → 헤더 1개만 생성해야 함
  "--------------------------------------------------------
  ...
  IF sy-subrc <> 0.
    MESSAGE |헤더 저장 실패: SUBRC={ sy-subrc } EBELN={ gs_002-ebeln }| TYPE 'E'.
    RETURN.
  ENDIF.
  ...
  LOOP AT gt_001 INTO gs_001.
    ...
    gs_003-ebeln = lv_ebeln.   "헤더 번호 연결

    INSERT zpdt19_005 FROM gs_003.
    IF sy-subrc <> 0.
      MESSAGE |아이템 저장 실패 (행 { sy-tabix }) EBELN={ gs_003-ebeln } EBELP={ gs_003-ebelp } SUBRC={ sy-subrc }| TYPE 'E'.
      RETURN.
    ENDIF.
  ...
  COMMIT WORK.
  MESSAGE |PO { lv_ebeln } 저장 성공| TYPE 'S'.
